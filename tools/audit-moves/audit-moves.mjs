 ...
 * チャンピオンズ独自の調整があった場合も、実機を見て手で直すしかない。
 */
...
for (const m of lib) {
  ...
  const x = expected.get(m.name);
  if (!x) { problems.push(`出典に無い技名: ${m.name}`); continue; }
  if (x.type !== m.type) problems.push(`タイプ: ${m.name} アプリ=${m.type} 出典=${x.type}`);
  if (x.power !== m.power) problems.push(`威力: ${m.name} アプリ=${m.power} 出典=${x.power}`);
  if (x.cat !== m.cat) problems.push(`分類: ${m.name} アプリ=${m.cat} 出典=${x.cat}`);
