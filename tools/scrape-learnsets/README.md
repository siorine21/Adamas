...
GameWith 側を取得できた21系統と比較したところ、**18系統は完全一致**、
**威力・命中は全技一致**でした。習得技データそのものに誤りは見つかっていません。

差が残ったのは次の3系統です。出典が割れているので、**実機で確認できるまで動かしません**。

| 系統 | 差分 | 状況 |
| --- | --- | --- |
| ハッサム | `ダブルウイング` が GameWith にのみ | 片方の出典のみ。未反映 |
| メタグロス | 当方 `でんじは` / GameWith `でんじふゆう` | どちらかの誤記。未反映 |
| ドリュウズ | `きあいパンチ` `ばかぢから` `インファイト` `まきびし` `エアスラッシュ` `サイコカッター` `もろはのずつき` が当方にのみ | GameWith の掲載漏れの可能性。現状維持 |

`きりさく` は5系統すべてで GameWith 側にだけありました。レギュM-Cでの解禁技であり
AppMedia の表が未更新と判断できるため、`gen-learnsets.mjs` の `EXTRA` で追加しています。
